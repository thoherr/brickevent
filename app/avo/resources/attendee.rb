class Avo::Resources::Attendee < Avo::BaseResource
  ...
  self.search = {
    query: -> {
      query.where(
        "attendees.given_name LIKE ? OR attendees.family_name LIKE ? OR attendees.email LIKE ? OR attendees.nickname LIKE ? OR attendees.lug LIKE ?",
        *(["%#{params[:q]}%"] * 5)
      )
    }
  }
  ...
  def fields
    ...
    # Core Information
    field :given_name, as: :text,
      help: "Given name (Vorname)"
    field :family_name, as: :text,
    ...
    # pretix shop (see doc/pretix.md) - maintained by BrickEvent and the order import
    field :voucher_code, as: :text, readonly: true, hide_on: [:new, :edit, :index],
      help: "Voucher for the pretix shop, generated by BrickEvent"
    field :voucher_exported_at, as: :date_time, readonly: true, hide_on: [:new, :edit, :index]
    field :order_code, as: :text, readonly: true, hide_on: [:new, :edit, :index]
    field :order_position_id, as: :number, readonly: true, hide_on: [:new, :edit, :index]
    field :order_status, as: :text, readonly: true, hide_on: [:new, :edit]
    field :ticket_secret, as: :text, readonly: true, hide_on: [:new, :edit, :index]
    field :ticket_url, as: :text, readonly: true, hide_on: [:new, :edit, :index]
    field :order_imported_at, as: :date_time, readonly: true, hide_on: [:new, :edit, :index]
